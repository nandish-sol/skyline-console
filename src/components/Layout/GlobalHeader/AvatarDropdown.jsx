...
export class AvatarDropdown extends React.Component {
  componentDidMount() {
    this._onAvatarChange = () => { this.forceUpdate(); };
    window.addEventListener('xloud-avatar-changed', this._onAvatarChange);
  }

  componentWillUnmount() {
    if (this._onAvatarChange) window.removeEventListener('xloud-avatar-changed', this._onAvatarChange);
  }

  get _avatarUrl() {
    return typeof localStorage !== 'undefined' ? localStorage.getItem('xloud_avatar') : null;
  }
    ...
    return (
      <HeaderDropdown overlay={menuHeaderDropdown}>
        <div className={`${styles.action}`}>
          {this._avatarUrl
              ? <Avatar src={this._avatarUrl} size={32} style={{ cursor: 'pointer' }} />
              : <Button shape="circle" icon={<UserOutlined />} className={styles.avatar} />
          }
        </div>
      </HeaderDropdown>
    );
